...
function readMessages(){
    ...
    ref.orderByChild('date').on("value", function(data){
        let objKey = Object.keys(data.val());
        for(obj in objKey){
            let key = objKey[obj];
            let a = document.getElementById("mensagem");

            a.innerHTML += '<a href="#" class="list-group-item"><b>'+data.val()[key].name+' <span class="badge">'+data.val()[key].date+'</span></b></br>'+
